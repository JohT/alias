...
public class TypeAliasAnnotationProcessorTest {
	...
	public void propertyFileShouldContainTypeAliasName() throws IOException {
		...
		files.assertContains("OrgExampleAliasClass=" + typeAliasAnnotatedClass.getJavaFileName(),
				DEFAULT_PROPERTYFILENAME);
	...
	public void propertyFileShouldContainAllAliasNames() throws IOException {
		...
		compileWithPresetProperties(orgExampleClass.createJavaFile(), orgAnotherClass.createJavaFile(),
				orgClass.createJavaFile()).to(directory.getRoot()).executeProcessor(processor);
		FileCollector files = FileCollector.collectFilesFromRoot(directory.getRoot());
	...
	public void resourceBundleFileShouldContainAliasName() throws IOException {
		...
		files.assertContains("\"OrgAliasClass\", " + orgClass.getJavaFileName() + ".class", "org",
				"AliasResourceBundle.java");
	}
	...
	public void propertyFileShouldContainHeaderComments() throws IOException {
		...
		compileWithPresetProperties(typeAliasAnnotatedClass.createJavaFile()).to(directory.getRoot())
				.executeProcessor(processor);
		FileCollector files = FileCollector.collectFilesFromRoot(directory.getRoot());
	...
	public void propertyFileShouldContainAliasNames() throws IOException {
		...
		compileWithPresetProperties(innerConfig, anotherInnerConfig, noConfigOuter.createJavaFile(),
				configuredInner.createJavaFile()).to(directory.getRoot()).executeProcessor(processor);
		FileCollector files = FileCollector.collectFilesFromRoot(directory.getRoot());

		// The default properties should only contain the types,
		// that doesn't belong to a configured (annotated package-info.java) package.
	...
	public void propertyFileShouldOnlyContainClassesWithinTheSameCustomizedPackage() throws IOException {
		...
		compileWithPresetProperties(packageInfo, withinPackage.createJavaFile(), outsidePackage.createJavaFile())
				.to(directory.getRoot()).executeProcessor(processor);
		FileCollector files = FileCollector.collectFilesFromRoot(directory.getRoot());

		files.assertContains("OrgExampleAliasClass=" + withinPackage.getJavaFileName(), "org", "example",
				"OrgExampleAlias.properties");
